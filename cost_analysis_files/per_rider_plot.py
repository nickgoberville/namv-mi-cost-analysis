#! /usr/bin/env python3

import glob
import pandas as pd
from plot_code import rider, YTD
import os, sys
namvi_mi_root = os.getcwd()+'/..'
sys.path.append(namvi_mi_root)
from cost_model.cost_model import * 
import process_sim_results as sim
import numpy as np
import matplotlib.pyplot as plt
from cost_analysis import line_plots

# Get modes (normal, AV_SD, AV_FM, AV_full)
modes = read_json('modes.json')     # defined in cost_model.cost_model

# Define different drivetrains
drive_trains = ['_EV', '_ICE']        #TODO REMOVE THE COMMENTED PART IN HERE

# cost per rider plot
fig_rider = plt.figure()
fig_rider.suptitle('Cost per Rider')
ax_rider = []
ax_rider.append(fig_rider.add_subplot(121))
ax_rider.append(fig_rider.add_subplot(122))

fig_rider_axes_set = False


# plot line params
plot_params = {'_EV': ['g', 'g:', 'g--', 'g*-'],
                '_ICE': ['r', 'r:', 'r--', 'r*-']}
colors = [(77/255, 148/255, 255/255),
          (255/255, 51/255, 51/255),
          (102/255, 153/255, 0/255),
          (91/255, 91/255, 141/255),
          (80/255, 80/255, 150/255)]
c1 = colors[0]
c2 = colors[1]
c3 = colors[2]
c4 = colors[3]

#Setup per rider plot
barWidth = 0.2
r1 = np.arange(4)      #Spacing for non AV
r2 = [x + barWidth for x in r1]     #Spacing for AV w/ SD
r3 = [x + barWidth for x in r2]     #Spacing for AV w/ FM
r4 = [x + barWidth for x in r3]     #Spacing for AV w/o operator

# loop each drive train
for enum, drive_train in enumerate(drive_trains):
    vehicles = pd.read_csv('vehicles'+drive_train+'.csv')   # extract vehicle models from csv file
    c = plot_params[drive_train]
    labels_set = False
    rider_vals = {'normal': [],
            'AV_SD': [],
            'AV_FM': [],
            'AV_full': []}
    # loop each vehicle size (S, M, L, XL)
    for index in vehicles.index:
        sim_results_path = 'sim_runs/scenario_'+str(index+1)+'/sim_results/'+vehicles.vehicle[index]+drive_train+'.csv'
        sim_results_df = pd.read_csv(sim_results_path)   # get simulation results for vehicle
        sim_results = sim.Results(sim_results_df)           # Process simulation results
        yearly_miles = np.sum(sim_results.daily_dist)       # Get yearly miles
        yearly_riders = np.sum(sim_results.daily_rides)     # Get yearly_riders

        # loop each mode
        for i, mode in enumerate(modes):
            label = drive_train+'_'+mode
            label = label[1:]
            # Get vehicle cost model
            vehicle = model(vehicles, index, yearly_miles, yearly_riders, mode=mode, inflation=False)

            #ytd, cash_flow = vehicle.total()
            #cost_per_rider = ytd[-1]/(yearly_riders*vehicle.time[-1])

            ytd, cash_flow = vehicle.per_rider()
            cost_per_rider = ytd[-1]
            rider_vals[mode].append(cost_per_rider)

            rider_vals[mode].append(cost_per_rider)

    fig_YTD_axes_set = True # set axes set to true
    
    #print(rider_vals)
    #plt.show() # YTD_plot

    p1 = ax_rider[enum].bar(r1, rider_vals['normal'], width=barWidth-.05, color=c1, edgecolor='k', label='Purchase')
    p2 = ax_rider[enum].bar(r2, rider_vals['AV_SD'], width=barWidth-.05, color=c2, edgecolor='k',label='Purchase')
    p3  = ax_rider[enum].bar(r3, rider_vals['AV_FM'], width=barWidth-.05, color=c3, edgecolor='k',label='Purchase')
    p4 = ax_rider[enum].bar(r4, rider_vals['AV_full'], width=barWidth-.05, color=c4, edgecolor='k',label='Purchase')

    ticks = (np.asarray(r2) + np.asarray(r3)) / 2
    ax_rider[enum].set_xticks(ticks)
        #labels = ['non-AV','AV w/ SO','AV w/ FM', 'Fully AV']
    xlabels = ['Small', 'Medium', 'Large', 'X-Large']

    ax_rider[enum].set_xticklabels(xlabels, fontsize=20)
        #ax.set_xticklabels((r1, r2, r3, r4), xlabels)
        
    ax_rider[enum].set_yticklabels(np.round(np.arange(0, 1.2, 0.2), 2), fontsize=20)

    legs = ['Non-AV', 'AV w/ Safety Operator', 'AV w/ Fleet Manager', 'Full AV']
    ax_rider[enum].legend((p1[0],p2[0],p3[0],p4[0]), legs, fontsize=20)
    ax_rider[enum].yaxis.grid(linestyle=':', color='k', linewidth=2)
    ax_rider[enum].set_ylabel('Cost ($/rider)')
    ax_rider[enum].set_title(drive_train[1:])
plt.show()