...
def yearly_op_maint(vtype, is_AV, teleops, t=1, fm=p.assumptions['Fleet size'], miles=p.assumptions['Miles/year']):
    return operation(vtype, is_AV, teleops, t, miles) + maintenance(vtype, is_AV, teleops, t, miles) + operator(is_AV, teleops, t, fm)

def iterative_total(vtype, is_AV, teleops, switch_y, t=p.assumptions['Years'], fm=p.assumptions['Fleet size'], miles=p.assumptions['Miles/year'], scale=1000):
    tot = []
    baseline_yearly = yearly_op_maint('BEV', 'S', False, fm=fm, miles=miles) / scale
    baseline_initial = purchase('BEV', 'S', False) / scale
    tot.append(purchase(vtype, is_AV, teleops)/scale - baseline_initial)
    time = 0
    while time < t:
        if time < switch_y:
            tot.append(tot[time] + yearly_op_maint(vtype, is_AV, teleops, fm=fm, miles=miles)/scale - baseline_yearly)
            time += 1
        elif time >= switch_y:
            if (time-switch_y) == 0:
                telep_purch = p.assumptions['teleops']/scale
            tot.append(tot[time] + yearly_op_maint(vtype, 'Y', True, fm=fm, miles=miles)/scale - baseline_yearly + telep_purch)
            time += 1
            telep_purch=0
    return tot
