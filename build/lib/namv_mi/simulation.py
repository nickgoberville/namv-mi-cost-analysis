...
class Shuttle:
    def __init__(self, vehicle, sim_params):
        self.riders = []        
    ...
    def next_stop(self):
        self.pos += 1
        if self.pos > self.sim_params['stops']-1:
            self.pos = 0
...
class Stop:
    def __init__(self, id, sim_params):
        self.id = id
        self.rider_list = []
...
class Simulator:
    def __init__(self, vehicle, sim_params):
        self.sim_params = sim_params
        # initialize Shuttle
        self.shuttle = Shuttle(vehicle, sim_params)
        ...
        for i in range(sim_params['stops']):
            self.stops.append(Stop(i, self.sim_params))

        # initialize riders
        for i in range(self.sim_params["initial riders"]):
            self.add_rider()

    def add_rider(self):
        rider = Rider(self.sim_params)
    ...
    def reset(self, start_riders):
        # Reset stops
        for i in range(self.sim_params['stops']):
...
def main(vehicle, sim_params, real_time=False, verbose=False, time_factor=10):
    # initialize sim
    sim = Simulator(vehicle, sim_params)
